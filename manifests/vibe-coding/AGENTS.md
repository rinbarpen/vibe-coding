...
- **`code-reviewer`**: For pre-commit quality checks and **automated test generation**.
- **`shell`**: For complex environment or Git operations.
- **`ci-watcher`**: For monitoring GitHub CI status and reporting failures.
- **`test-runner`**: For executing tests and ensuring coverage before and after commit.

## GitHub Automation

- Use `gh` CLI for all repository interactions (PRs, issues, checks).
- Before merging, ensure `ci-watcher` reports passing status.
- Use `gh pr create --fill` to quickly draft PRs with commit context.
...
- **Open Source**: Follow standards in `manifests/vibe-coding/standards/open-source-standards.md`.
